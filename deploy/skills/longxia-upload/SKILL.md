...
2. 上传前必须先判断登录是否还能复用。
3. 上传前必须先生成标题、文案、标签，且生成时要体现人物性格、情绪和关系感，不能只写通用套话。
4. 用户只说“先看看登录”“先补登录”时，只做登录，不自动开始发布。
5. 用户要求“登录后还要确认再发布”时，登录成功后停住。
6. 批量任务里某个平台登录失效时，标记跳过，不阻塞其他平台。
7. 每次回传结果必须说明：是否成功、关键日志节点、失败原因或跳过原因。
8. 若日志只有“已点击发布”但没有最终成功信号，不能算成功。
...
1. 先判断素材里人物最明显的性格或情绪，不要跳过这一步。
2. 标题要让用户一眼看到这条内容的“人物感”或“情绪感”，而不是每次都写成同一种氛围句。
3. 文案要同时满足：符合用户行业和视频风格、带当前 AI 的表达气质、能看出人物状态。
4. 如果画面里有人物，优先写人物的性格、情绪、关系感、反差点，不要只写空泛感受。
5. 不能连续复用同样的句式模板，例如反复出现“今日份”“又被治愈了”“记录一下”这种固定开头。
6. 用户没有明确给标题/文案时，默认先生成再给用户确认；不要直接拿旧文案复用。
